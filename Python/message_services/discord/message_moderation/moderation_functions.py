...
async def check_age(bot, message):
    ...
    if match1 or match2:
        ...
        if idade == 0:
            ...
            return True
    if bool(re.search(r'\b(tenho)?\s?(\d{1,2})\s?(anos)\b',message.content,re.IGNORECASE)) != bool(re.search(r'\b(\d{2}/\d{2}/\d{4}|\d{2}\s+de\s+(\w*)+\s+de\s+\d{4})\b',message.content,re.IGNORECASE)):
